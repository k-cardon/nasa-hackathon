...
function return_data_by_weather_condition(weather_condition, weather_data) {
	...
	if (weather_condition === 'Sunny') {
		for (let weather of weather_data) {
			wanted_weather_data.push([
				weather.location,
				weather.CLOUD_AMT_DAY,
				weather.PRECTOTCORR,
				weather.T2M
			]);
		}
		...
		return wanted_weather_data.sort((next, previous) => previous[2] - next[2]).reverse();
	}
...
export const actions = {
	default: async ({ request }) => {
		...
		for (let city of cities) {
			let city_url = `https://power.larc.nasa.gov/api/temporal/monthly/point?start=2020&end=2025&parameters=${params}&community=ag&longitude=${city[1]}&latitude=${city[0]}&format=JSON`;
